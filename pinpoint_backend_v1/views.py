...
def signup_handler(request):
  if request.method == "POST":
    if User.objects.filter(username=request.data['username']).exists():
      ...
      if 'profpicurl' not in request.data:
        profpicurl = ''
      else:
        profpicurl = request.data['profpicurl']
      
      if 'tagline' not in request.data:
        tagline = ''
      else:
...
def add_pin_handler(request):
  if request.method == "POST":
    if not Pin.objects.filter(address=request.data['address'], username=request.data['username']).exists():
      ...
      new_pin = Pin(address=address, latitude=latitude, longitude=longitude, name=name, username=username, rating=rating)
...
def create_collab_group(request):
  if request.method == "POST":
    ...
    for username in usernames:
      new_group_member = CollabGroup(group_id=dt_string, username=username)
